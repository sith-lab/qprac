...
for mitigation in mitigation_list:
    ...
    for result_filename in result_list:
        ...
        if not result_filename.startswith("wb_"):
            continue
        ...
        wb_th = result_filename.split("_")[6]
        ...
        for line in result_file.readlines():
            if ("qprac_wb_counter_writes" in line):
                found = True
                wb_writes = int(line.split(" ")[-1])
...
df = df.pivot(index=['workload', 'Cache_size'], columns=['mitigation'], values='writes').reset_index()
...
geomean_df = add_arithmetic_mean_rows(df)
...
# Save the CSV file
geomean_df.to_csv(os.path.join(csv_dir, 'QPRAC_4_and_256_WB.csv'), index=False)
...
def add_geomean_rows(df):
    ...
    for PRAC_level in df['PRAC_level'].unique():
        for suite_name, workloads in benchmark_suites.items():
            suite_df = df[(df['workload'].isin(workloads)) & (df['PRAC_level'] == PRAC_level)]
            if not suite_df.empty:
                ...
                # Create a new row
                geomean_row = {'PRAC_level': PRAC_level, 'workload': suite_name, **geomeans}
...
def add_all_workloads_geomean_rows(df):
    ...
    for PRAC_level in df['PRAC_level'].unique():
            Channel_interface_df = df[(df['PRAC_level'] == PRAC_level)]
            geomean_values = {}
            ...
            geomean_row = {'PRAC_level': PRAC_level, 'workload': 'All (57)', **geomean_values}
            geomean_rows.append(geomean_row)  # Append to the list
...
# Save the CSV file
geomean_df.to_csv(os.path.join(csv_dir, 'QPRAC_PRAC_Level_Results.csv'), index=False)
...
mitigation_list = ["Baseline", "QPRAC", "EA-QPRAC+1Proactive_per_4tREFI", "EA-QPRAC+1Proactive_per_2tREFI", "EA-QPRAC+1Proactive_per_1tREFI"]
...
def add_geomean_rows(df):
    ...
    for PSQ_size in df['PSQ_size'].unique():
        for suite_name, workloads in benchmark_suites.items():
            suite_df = df[(df['workload'].isin(workloads)) & (df['PSQ_size'] == PSQ_size)]
            if not suite_df.empty:
                ...
                # Create a new row
                geomean_row = {'PSQ_size': PSQ_size, 'workload': suite_name, **geomeans}
...
def add_all_workloads_geomean_rows(df):
    ...
    for PSQ_size in df['PSQ_size'].unique():
            Channel_interface_df = df[(df['PSQ_size'] == PSQ_size)]
            geomean_values = {}
            ...
            geomean_row = {'PSQ_size': PSQ_size, 'workload': 'All (57)', **geomean_values}
            geomean_rows.append(geomean_row)  # Append to the list
...
mitigation_list = ["QPRAC", "EA-QPRAC+1Proactive_per_4tREFI", "EA-QPRAC+1Proactive_per_2tREFI", "EA-QPRAC+1Proactive_per_1tREFI"]

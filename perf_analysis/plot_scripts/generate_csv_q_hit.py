...
for mitigation in mitigation_list:
    ...
    for result_filename in result_list:
        ...
        for line in result_file.readlines():
            if ("qprac_q_hits" in line):
                q_hits = int(line.split(" ")[-1])
            if ("qprac_q_misses" in line):
                q_misses = int(line.split(" ")[-1])

        if ((q_hits + q_misses) == 0):
            print(psq_size, workload)
        q_hit_rate = q_hits / (q_hits + q_misses)
        ...
        new_row = pd.DataFrame({
            ...
            'Q_size': [psq_size],
            'PQ_hitrate': [q_hit_rate],
        })
        df = pd.concat([df, new_row], ignore_index=True)

df_hit = df.pivot(index=['workload', 'Q_size'], columns=['mitigation'], values='PQ_hitrate').reset_index()
    ...
    for qsize in df['Q_size'].unique():
        for suite_name, workloads in benchmark_suites.items():
            ...
            if not suite_df.empty:
                ...
                # Create a new row
                geomean_row = {'Q_size': qsize, 'workload': suite_name, **geomeans}
                geomean_rows.append(geomean_row)  # Append to the list
...
def add_all_workloads_geomean_psq_rows(df):
    ...
    for qsize in df['Q_size'].unique():
            ...
            # Create a new row for the combined results
            geomean_row = {'Q_size': qsize, 'workload': 'All (57)', **geomean_values}
            geomean_rows.append(geomean_row)  # Append to the list

...
# Read the CSV file
csv_path = '../results/csvs/QPRAC_4_and_256_WB.csv'
...
# df_melted = pd.melt(df, id_vars=['workload'], value_vars=methods_interested, var_name='sizes', value_name='vals')
# print(df_melted)
...
df = df[df['workload'].isin(workloads_high_mpki)]
methods_interested = ['TH 4', 'TH 16', 'TH 64', 'TH 256']
df = df[df['Cache_size'].isin(methods_interested)]
df['Cache_size'] = pd.Categorical(df['Cache_size'], categories=methods_interested, ordered=True)
print(df)
...
fig.savefig(os.path.join(plots_dir, 'Figure_WB_RW.pdf'), dpi=600, bbox_inches='tight')

...
def get_multicore_run_commands():
    ...
    for config in multicore_params:
        mitigation, NBO, PRAC_level, PSQ_size, Targeted_REF_ratio, Cache_sizes, WB_sizes = config
        stat_str = "wb_" + make_stat_str(config[1:])
        ...
        for trace in traces:
            if mitigation == "QPRAC+Proactive-EA":
                ...
                add_mitigation(config, mitigation, NBO, PRAC_level, PSQ_size, Targeted_REF_ratio, Cache_sizes, WB_sizes)

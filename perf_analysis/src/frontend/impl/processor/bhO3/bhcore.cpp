...
// For debug
uint64_t BHO3Core::InstWindow::print_m_load () {
  return m_load;
}

void BHO3Core::InstWindow::print_m_addr_list () {
  std::cout<<"Current index: "<<m_tail_idx<<" addr_list: "<<m_addr_list[m_tail_idx]<<std::endl;
}


BHO3Core::BHO3Core(int id, int ipc, int depth, uint64_t num_expected_insts,
...
void BHO3Core::tick() {
  ...
  // For debug
  // if (s_insts_retired >= 165419310){
  //   std::cout<<"Current m_load: "<<m_window.print_m_load()<<std::endl;
  //   m_window.print_m_addr_list();
  // }

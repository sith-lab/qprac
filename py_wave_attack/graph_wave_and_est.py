...
ax.set_xticks(np.arange(x[0], x[-1] + 1, step=10000), )
...
for i in range(config_count//2):
    plt.plot(x, configList[i][0], lw=2, label=f"PRAC-{2 ** i}-EXACT")
for i in range(config_count//2):
    plt.plot(x, configList[i + config_count//2][0], lw=2, label=f"PRAC-{2 ** i}-EST")
...
plt.xlabel('Row Pool Size, $R_i$', fontsize=22)
plt.ylabel("$N_{online}$", fontsize=22)
# plt.title('Maximum Disturbance for Wave Attack Pattern\nBlast Radius = 2, N_BO=1', fontsize=20)
plt.savefig("N_Online_EST_VS_EXACT_PRAC_N.pdf", transparent=True, format="pdf", bbox_inches="tight")
plt.show()

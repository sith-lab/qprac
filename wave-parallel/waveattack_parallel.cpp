...
class MemoryController {
    ...
    public:
        MemoryController(int numRows, int aboAct, int k, int aboDelay, int alert_th, int simACTs, int gsize, int* shared_list, int* alert_list, int* unique_list)
            : dram(numRows, aboAct, k, aboDelay, alert_th), simulation_acts(simACTs), numRFM (k), current_act(0), activations_since_alert(0), gsize(gsize), num_alerts(0), shared_group_list(shared_list), shared_alert_list(alert_list), shared_uniquerow_list(unique_list) {
...
int main(int argc, char* argv[]) {
    if (argc != 4) {
        std::cerr << "Usage: " << argv[0] << " <NBO> <PRACN> <ABO_Delay>" << std::endl;
        return 1;
    }
    ...
    MemoryController mc(numRows, ABO_ACT, K, ABO_Delay, alert_th, simACTs, GSIZE, shared_group_list, shared_alert_list, shared_uniquerow_list);
